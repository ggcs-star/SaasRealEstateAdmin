...
export default function EditAmenities({ auth, project, amenities }) {
    const { data, setData, put, processing } = useForm({
        amenity_ids: (project.amenity_ids || []).map(id => String(id))
    });

    const selectedAmenities = (data.amenity_ids || []).map(id => String(id));
    ...
    const selectAll = () => {
        const allIds = amenities.map(item => String(item._id));
        setData('amenity_ids', allIds);
    };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route("projects.update.amenities", project._id));
    };
                    ...
                    <div className="bg-white shadow-xl rounded-2xl p-8">
                        <div className="space-y-8">
                            ...
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h2 className="text-2xl font-bold text-gray-800">Select Project Amenities</h2>
                                    <p className="text-sm text-gray-500 mt-1">
                                        Choose amenities available in your project
                                    </p>
                                </div>
                            ...
                            <div className="flex items-center justify-between bg-indigo-50 border border-indigo-100 rounded-lg p-4">
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    <span className="text-sm text-gray-600">of {amenities?.length || 0}</span>
                                </div>
                            </div>
                            ...
                            {selectedAmenities.length > 0 && amenities?.length > 0 && (
                            ...
                            <div className="flex justify-between pt-6 mt-6 border-t border-gray-200">
                                ...
                                <button
                                    ...
                                    disabled={processing}
                                    className="inline-flex items-center px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors disabled:opacity-50"
                                >
                                    {processing ? (
                                        <>
                                        ...
                                        <>
                                            {selectedAmenities.length > 0 ? (
                                                <>Update ({selectedAmenities.length})</>
                                            ) : (
                                                'Save (No Amenities)'
                                            )}
